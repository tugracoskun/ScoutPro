    ...
    steps.forEach((step) => {
        ...
        stepsHTML += `
            <div class="absolute" style="left: ${step.pos.x}%; top: ${step.pos.y / 2}%; transform: translate(-50%, -50%); z-index: 10;">
                <button ${disabledAttr} class="relative flex flex-col items-center group ${cursorClass}" onclick="${step.status === 'active' ? `app.navigate('academy-submodules', {stepId: ${step.id}})` : `window.alert(window.getLang() === 'en' ? 'This module is currently under construction.' : 'Bu bölüm yapım aşamasında.', 'error')`}">
                    ...
                    <div class="absolute ${tooltipPosClass} top-1/2 -translate-y-1/2 w-48 p-3 rounded-2xl bg-dark-900 border border-dark-800 shadow-xl opacity-0 group-hover:opacity-100 transition-opacity pointer-events-none z-20 hidden md:block">
                        <div class="text-xs font-bold text-white mb-1 flex items-center justify-between">
                            ${step.title}
                            ${step.status === 'locked' ? `<span class="text-[8px] bg-amber-500/10 text-amber-500 px-1.5 py-0.5 rounded ml-1 whitespace-nowrap uppercase border border-amber-500/20">${window.getLang() === 'en' ? 'Under Const.' : 'Yapım Aşamasında'}</span>` : ''}
                        </div>
