        ...
        <div class="space-y-8 fade-in max-w-7xl mx-auto">
            ...
            <div class="grid grid-cols-1 lg:grid-cols-3 gap-6">
                ...
                <div class="bg-dark-900 border border-dark-800 rounded-2xl p-1 relative overflow-hidden shadow-lg flex flex-col">
                    ${nextMatch ? `<div class="absolute top-0 left-0 w-full h-1 bg-gradient-to-r from-scout-500 to-blue-500"></div><div class="p-5 flex flex-col h-full"><h3 class="text-sm font-bold text-slate-400 uppercase tracking-wider mb-4 flex items-center gap-2"><i data-lucide="calendar-clock" class="w-4 h-4"></i> ${t('dash_next_match')}</h3><div class="flex-1 flex flex-col items-center justify-center text-center gap-3"><div class="text-xl font-black text-white leading-tight">${getTeam(nextMatch.homeId)} <span class="text-slate-600 text-sm font-medium mx-1">vs</span> ${getTeam(nextMatch.awayId)}</div><div class="bg-dark-950 px-4 py-2 rounded-lg border border-dark-800 text-scout-400 font-mono text-sm font-bold">${new Date(nextMatch.date).toLocaleString(window.getLang()==='en'?'en-US':'tr-TR', {weekday: 'short', hour: '2-digit', minute:'2-digit'})}</div></div><button onclick="app.navigate('matches')" class="w-full mt-4 py-2 bg-dark-800 hover:bg-dark-700 text-white text-xs font-bold rounded-lg transition-colors">${t('dash_go_to_list')}</button></div>` : `<div class="flex flex-col items-center justify-center h-full p-6 text-center"><div class="w-12 h-12 bg-dark-950 rounded-full flex items-center justify-center mb-3 text-slate-600"><i data-lucide="calendar-off" class="w-6 h-6"></i></div><p class="text-slate-500 text-sm">${t('dash_no_matches')}</p><button onclick="app.navigate('matches')" class="mt-3 text-scout-400 text-xs font-bold hover:underline">${t('dash_plan_match')}</button></div>`}
                </div>
            </div>
