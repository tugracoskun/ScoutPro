        ...
        <div class="lg:col-span-5 flex flex-col h-full">
            <div class="bg-dark-900/50 backdrop-blur rounded-3xl border border-dark-800 p-5 shadow-xl h-full flex flex-col">
                <div class="flex justify-between items-center mb-2">
                    ...
                    ${prevReport ? `<span class="text-[10px] text-slate-500 bg-dark-950 px-2 py-1 rounded border border-dark-700 flex items-center gap-1"><i data-lucide="arrow-left-right" class="w-3 h-3"></i> ${t('comparison')}: ${prevReport.date}</span>` : ''}
                </div>
            ...
            <div class="bg-dark-900/50 backdrop-blur rounded-3xl border border-dark-800 p-5 shadow-xl flex-1 flex flex-col min-h-0 overflow-hidden">
                ...
                <div id="content-media-notes" class="media-content ${activeTab === 'notes' ? 'flex' : 'hidden'} flex-col flex-1 h-full min-h-0">
                    <div id="social-list-${p.id}" class="flex-1 overflow-y-auto custom-scrollbar space-y-3 pr-2 mb-3">
                        ${socialNotes.length === 0 ? `<div class="text-slate-600 text-xs italic text-center py-8">${t('no_notes')}</div>` : socialNotes.map(n => `
                    ...
                    <div class="flex flex-col gap-2 mt-auto pt-2 border-t border-dark-800/50 shrink-0">
                        <input type="text" id="social-title-${p.id}" placeholder="${t('title_opt')}" class="w-full bg-dark-950 border border-dark-700 rounded-xl px-4 py-2 text-xs text-white outline-none focus:border-blue-500">
                        <div class="flex gap-2">
                            <input type="text" id="social-input-${p.id}" placeholder="${t('add_note')}..." class="flex-1 bg-dark-950 border border-dark-700 rounded-xl px-4 py-2 text-xs text-white outline-none focus:border-blue-500">
                ...
                <div id="content-media-videos" class="media-content ${activeTab === 'videos' ? 'flex' : 'hidden'} flex-col flex-1 h-full min-h-0">
                    <div id="video-list-${p.id}" class="flex-1 overflow-y-auto custom-scrollbar space-y-4 pr-2 mb-3">
                        ${videos.length === 0 ? `<div class="text-slate-600 text-xs italic text-center py-8">${t('no_videos')}</div>` : videos.map((v, i) => `
                    ...
                    <div class="flex flex-col gap-2 mt-auto pt-2 border-t border-dark-800/50 shrink-0">
                        <input type="text" id="video-title-${p.id}" placeholder="${t('video_title')}" class="w-full bg-dark-950 border border-dark-700 rounded-xl px-4 py-2 text-xs text-white outline-none focus:border-blue-500">
                        <div class="flex gap-2">
                            <input type="text" id="video-url-${p.id}" placeholder="${t('paste_link')}..." class="flex-1 bg-dark-950 border border-dark-700 rounded-xl px-4 py-2 text-xs text-white outline-none focus:border-blue-500">
