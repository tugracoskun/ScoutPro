// --- YENİ RAPORLAMA MODÜLÜ (V4.3 - Alignment Fix & Color Slider) ---
...
ScoutApp.prototype.renderNewReport = function(c) {
    ...
    const nationalTeams = this.state.data.teams.filter(t=>t.type==='national' && (!this.state.newReport.nationality || t.countryId == this.state.newReport.nationality)).map(t=>({val:t.id, txt:this.getTeamName(t.id), icon: t.logo}));
...
ScoutApp.prototype.updateRepStat = function(k, v, inputEl) {
    const val = parseInt(v);
    this.state.newReport.stats[k] = val;
    ...
    // Slider'ın kendi rengini (accent-color) değiştir
    if(inputEl) {
...
ScoutApp.prototype.updateBirthDate = function(val) {
    ...
    if (tipsBox) {
        if (!isYouthTipsEnabled) {
            tipsBox.classList.add('hidden');
            tipsBox.innerHTML = '';
            return;
        }

        if (typeof AGE_SPECIFIC_KPI !== 'undefined') {
            let groupKey = null;
            ...
            if (groupKey) {
                ...
                tipsBox.innerHTML = `
                    <div class="flex items-start gap-3">
                        <div class="w-10 h-10 rounded-lg bg-scout-500/20 flex items-center justify-center text-scout-500 shrink-0 mt-1"><i data-lucide="lightbulb" class="w-5 h-5"></i></div>
                        <div class="flex-1">
